...
export default function InventoryReorderProPage() {
  ...
  const L = linksFor(locale);

  const t = {
    title: locale === 'es'
      ? 'Inventory & Reorder PRO (ES/EN)'
      : 'Inventory & Reorder PRO (ES/EN)',
    subtitle: locale === 'es'
    ...
    includes: locale === 'es'
      ? [
          'Quick Start Guide (ES/EN)',
          'Versión Google Sheets',
          'Archivo Excel editable (sin macros)',
        ]
      : [
          'Quick Start Guide (ES/EN)',
          'Google Sheets version',
          'Editable Excel (no macros)',
        ],
  };
      ...
      <section className="mt-8 bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
        ...
        <ul className="grid md:grid-cols-3 gap-3 text-slate-700">
          {t.includes.map((x) => (
            <li
              key={x}
              className="bg-white rounded-xl border border-slate-100 p-4 shadow"
            >
              {x}
            </li>
          ))}
